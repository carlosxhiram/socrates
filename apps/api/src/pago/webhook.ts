 ...
 *     se rechaza con 400, jamás se procesa. Sin STRIPE_WEBHOOK_SECRET configurado,
 *     verificarWebhook lanza → respondemos 503 (honesto: no hay con qué verificar)
 *     y CERO efectos.
...
export async function manejarWebhookStripe(c: Context) {
  // Sin secreto de webhook no hay forma de verificar la firma: 503 honesto,
  // cero efectos. (Distinto de una firma inválida, que es 400.)
  if (!process.env.STRIPE_WEBHOOK_SECRET?.trim()) {
    return c.json(
      { error: { codigo: "SIN_STRIPE", mensaje: "El cobro no está configurado en el servidor." } },
      503,
    );
  }
  ...
  try {
    ...
    return c.json({ error: { codigo: "FIRMA_INVALIDA", mensaje: "Firma no verificable." } }, 400);
  }
  ...
  try {
    ...
    return c.json({ error: { codigo: "NO_APLICADO", mensaje: "No se pudo aplicar." } }, 500);
  }
...
async function actualizarDesdeSuscripcion(sub: Stripe.Subscription): Promise<void> {
  ...
  // Ambos vienen de NUESTRA fila / de lo que nosotros pusimos en el checkout,
  // nunca de un campo manipulable del navegador.
