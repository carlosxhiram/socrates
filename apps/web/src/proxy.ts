...
// Rutas públicas: la landing (/), la página institucional (/nosotros) y el
// sign-in. Todo lo demás requiere sesión (cuando hay Clerk). /bienvenida queda
// protegida: exige sesión, pero el pago y la bienvenida los gestiona el propio
// recibimiento (portero server-side).
const esRutaPublica = createRouteMatcher(["/", "/nosotros", "/sign-in(.*)"]);
